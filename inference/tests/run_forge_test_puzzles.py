...
from agent.forge_search_engine_strategy import ForgeSearchEngineStrategy
from logic.zebra_benchmark import ZebraBenchmark
...
async def run_puzzle(puzzle_config: tuple[tuple[int, list[str]], str], model_name: str) -> tuple[str, bool, str]:
    ...
    try:
        ...
        solution = result_trace.solver_output # NOTE: not actually the solution, but for testing purposes, the solver output
        solver_code = result_trace.solver_exit_code
        if solver_code == 0 and solution and solution.strip() and solution.strip() != "(no solver output)":
