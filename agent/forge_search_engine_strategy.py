...
from logic.engine_strategy import EngineStrategy, SolverOutcome
from agent.logic_py_forge_harness_generator import LogicPyForgeHarnessGenerator
...
class ForgeSearchEngineStrategy(EngineStrategy):
    ...
    def generate_solver_invocation_command(self, solver_input_file: str) -> list[str]:
        ...
        return ["racket", solver_input_file, "-O", "run_sterling", "off"]
    ...
    def parse_solver_output(
        ...
        """
        return self.parse_solver_output_from_assert(exit_code, stdout, stderr)
        # return self.parse_solver_output_from_json(exit_code, stdout, stderr)

    def parse_solver_output_from_assert(self, exit_code: int, stdout: str, stderr: str) -> Tuple[SolverOutcome, Optional[str]]:
        # UNSAT case
        if exit_code != 0:
            self.__logger.error(
    ...
    def parse_solver_output_from_json(
        self, exit_code: int, stdout: str, stderr: str
    ) -> Tuple[SolverOutcome, Optional[str]]:
        """
        Interprets the result of the constraint solver subprocess invocation.
